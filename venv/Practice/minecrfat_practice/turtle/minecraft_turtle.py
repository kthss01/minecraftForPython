...
class MinecraftTurtle:
    ...
    def forward(self, distance):
        # get end of line
        x, y, z = self._findPointOnSphere(self.position.x, self.position.y, self.position.z,
                                          self.heading, self.verticalheading, distance)
        # move turtle forward
    ...
    def backward(self, distance):
        # get end of line
        x, y, z = self._findPointOnSphere(self.position.x, self.position.y, self.position.z,
                                          self.heading, self.verticalheading - 180, distance)
        # move turtle forward
        self._moveTurtle(x, y, z)
        ...
        if self.turtlespeed == 0 and self.flying:
            ...
            if self._pendown:
                self.mcDrawing.drawLine(
                    ...
                    targetX, targetY - 1, targetZ, self._penblock.id, self._penblock.data)
        else:
            ...
            for blockBetween in blocksBetween:
                ...
                # draw the pen
                if self._pendown:
                    self.mcDrawing.drawPoint3d(blockBetween.x, blockBetween.y - 1, blockBetween.z,
    ...
    def setverticalheadings(self, angle):
    ...
    def _roundXYZ(x, y, z):
        return int(round(x, 0)), int(round(y, 0)), int(roudn(z, 0))
